...
def start(update, context):
    update.message.reply_text(
        'Welcome to QuanBot, please type your QuanID /quanid')
    update.message.reply_text(
        'For unsubscribe from a notification id please type /unsub and the QuanID')
...
def getName(update, context):
    ...
    if(nome == settings.CLIENT_NAME):
        ...
        update.message.reply_text(
            'The Bot from {0} is {1}, to know better the execution state of your\
		bot access url, but we will keep you posted of the relevant information \
			through this channel'.format(
                nome, 'off' if settings._PAUSE_BOT else 'on'))
    ...
    if(qid == settings.CLIENT_NAME):
        for chat in chat_id:
            if(chat == update.message.chat_id):
                ...
                update.message.reply_text(
                    'Sucessfuly removed {0} from your notification list'.format(
                        settings.CLIENT_NAME))

# To Send custom message after user
# logged/bot1199841211:AAHi1GPjTQJ0dkBTTXSm8zHcQEUzIwBT
...
def send_message(msg):
    ...
    if(chat_id):
        bot = telegram.Bot(
            token="1199841211:AAHi1GPjTQJ0dkBTTXSm8zHcQEUzIwBThcs")
        for chat in chat_id:
...
def send_group_message(
        msg,
        bot_token=settings.TOKEN_MAIN_BOT,
        chat_id=-496268763):
    try:
...
def InitialiseBot():
    updater = Updater(
        "1242980549:AAHuWBWEo33pViTpHxMeeN97gCs5V7YHDlA",
        use_context=True)
