...
try:
    ...
    from pryno.util import settings
    settings.VALID_USERNAME_PASSWORD_PAIRS.update(
        {settings.CLIENT_NAME: settings.CLIENT_PWD})
...
if __name__ == '__main__':
    try:
        ...
        settings.VALID_USERNAME_PASSWORD_PAIRS.update(
            {settings.CLIENT_NAME: settings.CLIENT_PWD})
        settings.STM_INDICATOR_INIT = len(
            settings.BOT_NAME) + len(settings.BOT_VERSION) + 2
        settings.GRADLE_INDICATOR_INIT = len(
            settings.BOT_NAME) + len(settings.BOT_VERSION) + 2
        settings.STM_INDICATOR_END = len(
            settings.STM_INDICATOR) + len(settings.BOT_NAME) + len(settings.BOT_VERSION) + 1
        settings.GRADLE_INDICATOR_END = len(
            settings.BUY_INDICATOR) + len(settings.BOT_NAME) + len(settings.BOT_VERSION) + 1
